...
import {PSCircularButton} from '../atom/PSButton';
...
function StoryDetail(props: any) {
  ...
  function getAllStory() {
    try {
      ...
      axios
        .get('http://localhost:3000/api/docs/PROJECT', {headers})
        .then((response: any) => {
          // Handle the response data here
          console.log(response.data);
  ...
  React.useEffect(() => {
    ...
    socket.on('add_story_success', msg => {
      console.log('New message:', msg);
      setModalVisible(!isModalVisible);
    });
    socket.on('add_story_fail', msg => {
      console.log('New message:', msg);
      setModalVisible(!isModalVisible);
    });
    socket.on('arvind', msg => {
      console.log('New message:', msg);
    });
  }, [socket]);
      ...
      <FlatList
        ...
        renderItem={({item}) => (
          <TouchableOpacity
            ...
            onPress={() => navigation.push('PokerRoom', {story: item})}>
